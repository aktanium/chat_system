...
app.get(
    ...
    async (req, res) => {
        try {
            ...
            const result = await db.query(
                `
      SELECT m.id, m.conversation_id, m.sender_id, m.content, m.message_type, m.is_encrypted, m.expires_at,
...
app.post(
    ...
    async (req, res) => {
        try {
            const { conversationId } = req.params;
            const { content, is_encrypted = false, ephemeral_ttl = null } = req.body;
            const userId = req.user.userId;
            ...
            const result = await db.query(
                `
      INSERT INTO messages (conversation_id, sender_id, content, is_encrypted, ephemeral_ttl, expires_at)
      VALUES ($1, $2, $3, $4, $5, CASE WHEN $5::int IS NOT NULL THEN NOW() + ($5::int * INTERVAL '1 second') ELSE NULL END)
      RETURNING id, sender_id, content, is_encrypted, created_at, expires_at
    `,
                [conversationId, userId, content, is_encrypted, ephemeral_ttl],
            );
            ...
            if (content.startsWith('/ai ')) {
                setTimeout(async () => {
                    const botReply = `[AI Assistant]: I received your prompt: "${content.replace('/ai ', '')}". (Integration placeholder)`;
                    ...
                    if (botCheck.rows.length === 0) {
                        const pass = await bcrypt.hash('botpassword123', 10);
                        const newBot = await db.query(
                            'INSERT INTO users (username, email, password_hash) VALUES ($1, $2, $3) RETURNING id',
                            ['AIBot', 'bot@system.local', pass],
                        );
                        botId = newBot.rows[0].id;
                    } else {
                        botId = botCheck.rows[0].id;
                    }

                    // Make sure bot is in conversation
                    await db.query(
        ...
        `,
                        [conversationId, botId, botReply],
                    );
                    const botMsg = botMsgRes.rows[0];
...
app.post(
    '/api/messages/:messageId/mark-as-read',
    authenticateToken,
    async (req, res) => {
        try {
            const { messageId } = req.params;
